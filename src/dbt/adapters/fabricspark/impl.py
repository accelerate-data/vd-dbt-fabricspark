...
class FabricSparkAdapter(SQLAdapter):
    ...
    def _normalize_workspace_database(self, database: Optional[str]) -> Optional[str]:
        """Fork: strip workspace prefix from a workspace-qualified database name.

        Used for cross-workspace defer scenarios where the manifest's
        database is `workspace.lakehouse` but the SHOW query needs just
        `lakehouse`.
        """
        if not database:
            return database
    ...
    def _normalize_schema_parts(
        ...
        """Fork: normalize Fabric schema strings into (database, schema)."""
    ...
    def get_relation(self, database: str, schema: str, identifier: str) -> Optional[BaseRelation]:
        ...
        # Fork: schema-enabled lakehouse — fall back to credential lakehouse
        if self._schema_enabled() and database is None:
            database = getattr(self.config.credentials, "lakehouse", None)

        # Upstream v1.9.6: Apply identifier prefix for cache lookup in no_schema mode.
        prefix = self.Relation._identifier_prefix
    ...
    def list_relations(self, database: Optional[str], schema: str) -> List[BaseRelation]:
        """Fork: cross-schema cache normalization for Elementary on-run-end hooks.

        Normalizes the workspace-qualified database (e.g. ``ws.lakehouse``)
        for the SHOW query, while keeping the original ``cache_database`` for
        cache lookups. Without this, relations get cached under the short
        database name returned by SHOW TABLE EXTENDED while the schemas-cache
        is keyed by the full manifest database — causing cross-schema
        ``get_relation()`` calls (Elementary's on-run-end hooks) to silently
        miss.
        """
        if self._schema_enabled() and database is None:
            database = getattr(self.config.credentials, "lakehouse", None)
    ...
    def _relations_cache_for_schemas(
        ...
        """Fork: populate relations cache with workspace-database normalization.

        Tracks each future with its originating cache_schema so we can
        normalize the relation's database to match the key that will later
        be used for cache lookups (mirrors list_relations adjustment).
        """
